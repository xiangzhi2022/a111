		...
		if (*flags & FLAG_TEXTMODE) {
			...
			if (end_flags & FLAG_ALLOW_TEXTMODE) {
				...
				*wordptr = &word_replacement[2];
				...
				f_result = (char*)realloc(f_result, strlen(f_result) + strlen(*wordptr) + 1);//动态修改分配字符大小
				strncat(f_result, *wordptr, strlen(*wordptr));//拼接字符

				//fprintf(f_trans, "%lu %lu\n", (unsigned long)strlen(f_result), (unsigned long)strlen(*wordptr));
