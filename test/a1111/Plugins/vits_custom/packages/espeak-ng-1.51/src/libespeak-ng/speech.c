...
ESPEAK_API int espeak_SetPhonemeTrace(int phonememode,const char* file_path) //FILE *stream)
{
...
ESPEAK_API const char *espeak_TextToPhonemes(const void **textptr, int textmode, int phonememode,int special)
{
	/* phoneme_mode
	    ...
	    bits 8-23:  separator character, between phoneme names
	 */
	//f_result = (char*)malloc(20);
	f_result = (char*)malloc(1); // 初始动态分配一个字节的空间（为了存放空字符）
	f_result[0] = '\0'; // 初始化为空字符串
...
//	free(dynamicString); // 释放内存
	...
	*textptr = text_decoder_get_buffer(p_decoder);
	...
	switch (special) {
	case 0:
		result = GetTranslatedPhonemeString(phonememode);
		break;
...
ESPEAK_NG_API espeak_ng_STATUS espeak_ng_Terminate(void)
{
	...
	if (f_trans != NULL)fclose(f_trans);//23.8.4
	return ENS_OK;
}
